...
module.exports = {
    ...
    async execute(client, message, args, color) {
        ...
        if (owner.get(`owners.${message.author.id}`) || config.app.inspi.includes(message.author.id) === true) {
        ...
        if (owner.get(`owners.${message.author.id}`) || message.member.roles.cache.has(p3.fetch(`perm3_${message.guild.id}`)) || config.app.inspi.includes(message.author.id) === true) {
